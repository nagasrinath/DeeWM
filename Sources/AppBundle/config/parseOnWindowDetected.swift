...
struct WindowDetectedCallbackMatcher: ConvenienceCopyable, Equatable {
    ...
    var debugJson: Json {
        ...
        if let duringDwmacStartup {
            resultParts.append("duringDwmacStartup=\(duringDwmacStartup)")
        }
...
private let matcherParsers: [String: any ParserProtocol<WindowDetectedCallbackMatcher>] = [
    ...
    "during-dwmac-startup": Parser(\.duringDwmacStartup, upcast(parseBool)),
]
