...
public func getMessageWindow(messageModel: MessageModel) -> some Scene {
    // Using SwiftUI.Window because another class in Dwmac is already called Window
    SwiftUI.Window(messageModel.message?.title ?? dwmacAppName, id: messageWindowId) {
        MessageView(model: messageModel)
            .onAppear {
                // Set activation policy; otherwise, Dwmac windows won't be able to receive focus and accept keyboard input
...
public let messageWindowId = "\(dwmacAppName).messageView"
...
public struct Message: Hashable, Equatable {
    ...
    init(type: MessageType = .config, title: String = dwmacAppName, description: String, body: String) {
        self.type = type
        self.title = title
