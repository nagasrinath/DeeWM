...
public let socketPath = "/tmp/\(dwmacAppId)-\(unixUserName).sock"
...
public func dieT<T>(
    ...
    let message =
        """
        Please report to:
            https://github.com/hillyu/Dwmac/discussions/categories/potential-bugs
            Please describe what you did to trigger this error

        Message: \(_message)
        Version: \(dwmacAppVersion)
        Git hash: \(gitHash)
    ...
    if !isUnitTest && isServer {
        showMessageInGui(
            filenameIfConsoleApp: recursionDetectorDuringTermination
                ? "dwmac-runtime-error-recursion.txt"
                : "dwmac-runtime-error.txt",
            title: "Dwmac Runtime Error",
            message: message,
        )
    }
