...
- `../Sources`.
  The majority of Dwmac source code. Managed by SPM `../Package.swift`
- `../Sources/AppBundle/`.
  Dwmac.app server. Technically, it's a SPM library that is exposed to `xcode-app-bundle-launcher`
- `../Sources/Cli/`.
...
- `../xcode-app-bundle-launcher/`.
  ...
  The Xcode project model is located in `../Dwmac.xcodeproj/` and generated by `../project.yml` "skeleton"
...
`dwmac` CLI binary is client. `Dwmac.app` is server. Client and server talk to each other via predefined UNIX file.
...
Command checklist:
- [ ] Documentation in `../docs/dwmac-*` and `../docs/commands.adoc`
