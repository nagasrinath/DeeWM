...
If you struggle to build Dwmac locally, you can also refer to [builds in GitHub Actions](https://github.com/hillyu/Dwmac/actions?query=branch%3Amain)
...
4.  If you want to build man pages, install Ruby >= 3.0. I recommend using [rbenv](https://github.com/rbenv/rbenv).
    ...
    -   Install asciidoctor using Ruby `bundler`. `cd Dwmac && bundler install`
5.  Install optional `xcbeautify` to make Xcode build logs readable. `brew install xcbeautify`

## 2. Create codesign certificate

If you want to run Dwmac as App Bundle (Dwmac.app) you need to create self-signed certificate that will be used to codesign Dwmac.
Release artifact is built as App Bundle.
If you only plan to build the debug version of Dwmac, you can run it from the terminal and custom certificate is not required.

1.  Open `Keychain Access.app`
2.  Menu -> `Keychain Access` -> `Certificate Assistance` -> `Create a Certificate...`
    -   Name: `dwmac-codesign-certificate`
...
-   `run-debug.sh` - Run Dwmac.app debug build.
-   `run-cli.sh` - Run `dwmac` in CLI. Arguments are forwarded to `dwmac` binary.
...
-   `build-shell-completion.sh` - Build shell completion to `.shell-completion`.
    You can test that the completion works properly by sourcing the file `source ./.shell-completion/zsh/_dwmac`
-   `generate.sh` - Regenerate generated project files. `Dwmac.xcodeproj` is generated, and some of the source files
    (the source files have `Generated` suffix in their names).

**Release build**
-   `build-release.sh` - Build release build to `.release` dir by using Xcode.
-   `install-from-sources.sh` - Build release build from sources and install it as `dwmac-dev` brew cask.
...
1.  To open the project in Xcode: File -> Open -> Choose `Package.swift` file instead of `Dwmac.xcodeproj`.
    It's better to open `Package.swift`, because SPM project is more lightweight.
    `Dwmac.xcodeproj` is only used in `*release*.sh` build scripts.
