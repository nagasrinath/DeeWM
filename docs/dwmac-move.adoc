...
[verse]
// tag::synopsis[]
dwmac move [-h|--help] [--window-id <window-id>] [--boundaries <boundary>] [--boundaries-action <boundary-action>] (left|down|up|right)
...
----
+
`v_master-stack` is an implicitly created container.
+
