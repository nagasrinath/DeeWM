...
export XCODEGEN_DWMAC_CODE_SIGN_IDENTITY="dwmac-codesign-certificate"
...
while test $# -gt 0; do
    case $1 in
        ...
        --codesign-identity) XCODEGEN_DWMAC_CODE_SIGN_IDENTITY="$2"; shift 2 ;;
...
public let dwmacAppVersion = "$build_version"
EOF
...
entries() {
    for file in docs/dwmac-*.adoc; do
        ...
        subcommand=$(basename $file | sed 's/^dwmac-//' | sed 's/\.adoc$//')
...
if test $generate_xcodeproj = 1; then
    export XCODEGEN_DWMAC_VERSION=$build_version
