...
cask_version=':latest' # Prevent 'Not upgrading dwmac, the latest version is already installed'
zip_root_dir="Dwmac-v$build_version"
...
cask "$cask_name" do # THE FILE IS GENERATED BY build-brew-cask.sh
  ...
  postflight do
    system "xattr -d com.apple.quarantine #{staged_path}/$zip_root_dir/bin/dwmac"
    system "xattr -d com.apple.quarantine #{appdir}/Dwmac.app"
  end

  app "$zip_root_dir/Dwmac.app"
  binary "$zip_root_dir/bin/dwmac"

  binary "$zip_root_dir/shell-completion/zsh/_dwmac",
      target: "#{HOMEBREW_PREFIX}/share/zsh/site-functions/_dwmac"
  binary "$zip_root_dir/shell-completion/bash/dwmac",
      target: "#{HOMEBREW_PREFIX}/etc/bash_completion.d/dwmac"
  binary "$zip_root_dir/shell-completion/fish/dwmac.fish",
