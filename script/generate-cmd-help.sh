...
for file in docs/dwmac-*.adoc; do
    subcommand=$(basename "$file" | sed 's/^dwmac-//' | sed 's/\.adoc$//' | sed 's/-/_/g')
    sed -n -E '/tag::synopsis/, /end::synopsis/ p' "$file" | \
        sed '1d' | \
        sed '$d' | \
        sed '/^$/ d' | \
        sed "1   s/^$dwmac_prefix/$____usage_prefix/" | \
        sed "2,$ s/^$dwmac_prefix/$_______or_prefix/" | \
